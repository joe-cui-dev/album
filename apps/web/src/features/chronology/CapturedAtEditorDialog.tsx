...
function EditState({ editor, snapshot, dateRef, timeRef, offsetRef }: { editor: CapturedAtEditor; snapshot: ReturnType<CapturedAtEditor["getSnapshot"]>; dateRef: RefObject<HTMLInputElement | null>; timeRef: RefObject<HTMLInputElement | null>; offsetRef: RefObject<HTMLInputElement | null> }) {
  ...
  return (
    <>
      ...
      <p className="mt-1 text-sm text-ink-muted" id="captured-at-editor-help">Enter the date and local time shown by the Photo. Leave UTC offset blank when it is unknown.</p>
      <div aria-live="assertive" className="mt-3 text-sm text-danger" role="alert">{snapshot.networkError}</div>
      <div className="mt-4 grid gap-4">
        <FieldError {...(errors.date ? { error: errors.date } : {})} id="captured-at-date-error" />
        <label className="grid gap-1" htmlFor="captured-at-date">Date <input aria-describedby={errors.date ? "captured-at-date-error" : undefined} aria-invalid={Boolean(errors.date)} className="rounded border border-control-line px-3 py-2" id="captured-at-date" onChange={(event) => editor.intents.change("date", event.target.value)} ref={dateRef} required type="date" value={draft.date} /></label>
        <FieldError {...(errors.time ? { error: errors.time } : {})} id="captured-at-time-error" />
        <label className="grid gap-1" htmlFor="captured-at-time">Time <input aria-describedby={errors.time ? "captured-at-time-error" : undefined} aria-invalid={Boolean(errors.time)} className="rounded border border-control-line px-3 py-2" id="captured-at-time" onChange={(event) => editor.intents.change("time", event.target.value)} ref={timeRef} required type="text" value={draft.time} /></label>
        <label className="grid gap-1" htmlFor="captured-at-resolution">Time includes
          <select className="rounded border border-control-line px-3 py-2" id="captured-at-resolution" onChange={(event) => editor.intents.change("timeIncludes", event.target.value)} value={draft.timeIncludes}>
...
function RevertState({ chronology, editor, saving }: { chronology: PhotoChronology; editor: CapturedAtEditor; saving: boolean }) {
  return <><h2 className="text-xl font-bold" id="captured-at-editor-title">Revert to original date and time?</h2><dl className="mt-4 grid grid-cols-[auto_1fr] gap-x-3 gap-y-2 text-sm" id="captured-at-editor-help"><dt>Current</dt><dd>{formatCapturedAt(chronology.active.capturedAt, "detail")}</dd><dt>Original</dt><dd>{formatCapturedAt(chronology.original.capturedAt, "detail")}</dd><dt>Original source</dt><dd>{capturedAtSourceLabel(chronology.original.source)}</dd></dl><div className="mt-6 flex justify-end gap-2"><button className="rounded px-3 py-2" disabled={saving} onClick={editor.intents.cancelRevert} type="button">Cancel</button><button className="rounded bg-danger px-3 py-2 font-semibold text-white disabled:opacity-50" disabled={saving} onClick={editor.intents.confirmRevert} type="button">{saving ? "Reverting…" : "Revert"}</button></div></>;
}
