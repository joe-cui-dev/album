...
test("traps Tab focus within the contextual Viewer", async ({ mock, page }) => {
  ...
  // No Previous/Next neighbours are present; the Slice 3 Zoom control is the last
  // focusable Viewer control, so Shift+Tab from the first must wrap to it.
  await page.keyboard.press("Shift+Tab");
  await expect(page.getByRole("button", { name: "View at 100%" })).toBeFocused();
